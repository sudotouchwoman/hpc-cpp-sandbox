...
void GpuDgemmHandle::setup(Backend backend_kind, int m, int n, int k) {
...
void GpuDgemmHandle::setStream(cudaStream_t s) { stream = s; }

void GpuDgemmHandle::synchronize() const { CHECK_CUDA(cudaStreamSynchronize(stream)); }

void GpuDgemmHandle::allocateDeviceBuffers() {
  if (!dA) {
...
void GpuDgemmHandle::setDeviceBuffers(double* deviceA, double* deviceB,
                                      double* deviceC) {
  dA = deviceA;
  dB = deviceB;
  dC = deviceC;
...
void GpuDgemmHandle::uploadAAsync(const double* A_host, int lda_host) {
  lda = lda_host;
  const size_t bytes =
...
void GpuDgemmHandle::uploadBAsync(const double* B_host, int ldb_host) {
  ldb = ldb_host;
  const size_t bytes =
...
void GpuDgemmHandle::uploadCAsync(const double* C_host, int ldc_host) {
  ldc = ldc_host;
  const size_t bytes =
...
void GpuDgemmHandle::downloadCAsync(double* C_host, int ldc_host) const {
...
void GpuDgemmHandle::execute(double alpha, double beta) const {
  switch (backend) {
    case Backend::Basic:
    ...
    case Backend::Unified:
      ...
      break;
    case Backend::CuBLAS:
      cublas::dgemm_impl_device(M, N, K, alpha, dA, lda, dB, ldb, beta, dC,
                                ldc, stream);
      break;
  }
}
